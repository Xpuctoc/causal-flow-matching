# Below code with modifications from Chao et al. "Interventional and Counterfactual Inference with Diffusion Models"
...
def exp_helper(models, int_var, true_model, scm_type, equations_type, seed, graph, column_order=None,
               dataset_params=None):
        ...
        for model_name in models:
            ...
            if model_name in ["VACA", "CAREFL"]:
                ...
                cur_cf_pred, vaca_gt_cf_full, _ = convert_data_to_pandas(data_loader.dataset.var_to_idx,
                                                                         *[x['all'] for x in
                                                                           model.get_counterfactual_distr(
                                                                               **input_params)])
                vaca_gt_cf = vaca_gt_cf_full[descendants]
                assert np.max(np.abs(gt_cf - vaca_gt_cf))[
                           0] < 1e-4, f"Inconsistent counterfactual values from {model_name} and DCM"

                if model_name == "CAREFL":
                    input_params.pop("num_batches")
                cur_int_pred, _ = convert_data_to_pandas(data_loader.dataset.var_to_idx,
                                                         *[x['all'] for x in
                                                           model.get_interventional_distr(**input_params)])
            else:
                cur_cf_pred = reindex_columns(column_order,
                                              counterfactual_samples(model, intervention, observed_data=factual))
                cur_int_pred = reindex_columns(column_order, interventional_samples(model, intervention,
                                                                                    num_samples_to_draw=num_int_samples))
...
def all_exp(equations_type, scm_type, use_vaca):
    ...
    for i in range(num_initializations):
        ...
        cafm_model = create_cafm_model(scm_type, params_cafm, graph.copy())
        diff_model = create_diff_model(scm_type, params_diff, graph.copy())
        ...
        cy.fit(cafm_model, train_df)
        cy.auto.assign_causal_mechanisms(diff_model, train_df, override_models=True)
        cy.fit(diff_model, train_df)

        models = {
            model_names[0]: cafm_model,
            model_names[1]: diff_model
        }
        if use_vaca:
        ...
        for j, int_var in enumerate(all_int_var):
            int_mmds, cf_mses = exp_helper(models, int_var, true_model, scm_type, equations_type, seed, graph,
                                           column_order, dataset_params)
            for model_name in model_names:
        ...
        summary_metrics.to_csv(f"experiments/metric_summaries/{scm_type}_{equations_type}_{n}_{num_epochs}.csv",
                               index=False)
...
if __name__ == '__main__':
    ...
    use_vaca = False
    save_preds = True
    params_cafm = {'num_epochs': num_epochs,
                   'lr': 1e-4,
                   'batch_size': 64,
                   'hidden_dim': 128,
                   'weight_decay': 0,
                   'clip': False,
                   'verbose': False}
    params_diff = {'num_epochs': num_epochs,
                   'lr': 1e-4,
                   'batch_size': 64,
                   'hidden_dim': 128,
    ...
    if use_vaca:
        sys.path.append('vaca/')
        from VACA_helper import *

    scm_list = scm_types
    equations_list = equations_types
    all_summary_metrics = {}
    ...
    for equations_type in equations_list:
        for scm_type in scm_list:
            print(
                f"\nEvaluating {scm_type} {equations_type} Data with n={n} and {num_epochs} epochs for CaFM, DCM\n-------------------------------\n")
