    ...
    for metric in metrics:
        for col in sorted_cols:
            if metric in col:
                ...
                print(
                    f"{col.replace('_', ' '):<30} {np.mean(vals) * scaling:>9.4f} \u00B1{np.std(vals) * scaling:>8.4f}")
        print("")
...
def initialize_summary_metrics(scm_type, model_names, num_initializations):
    ...
    if scm_type is not "random":
        summary_names.extend(
            [model + "_" + metric + "_" + int_var for metric in ["Int_MMD", "CF_MSE"] for model in model_names for
             int_var in all_int_var])
    else:
        summary_names.extend(
            [model + "_" + metric + "_" + str(ind) for metric in ["Int_MMD", "CF_MSE"] for model in model_names for ind
             in range(1, 4)])
