...
from cafm.cafm import CausalDiffusionModel, create_model_from_graph, CausalFM, create_cafm_model_from_graph
...
def select_struct_and_noise(equations_type, scm_type, weights=None, graph=None):
    ...
    if scm_type == "sachs":
        ...
        def sachs_empirical(node):
            ...
            empirical_data = (empirical_data - np.min(empirical_data)) / (
                        np.max(empirical_data) - np.min(empirical_data)) * 2 - 1
...
def get_graph(scm_type):
    ...
    if scm_type == "triangle":
        ...
        g = nx.DiGraph([('x01', 'x02'), ('x01', 'x03'), ('x02', 'x03'),
                        ...
                        ('x05', 'x10'), ('x05', 'x06'), ('x05', 'x08'), ('x05', 'x09'), ('x05', 'x10'), ('x05', 'x11'),
                        ('x05', 'x07'),
...
def create_cafm_model(scm_type, params, g=None):
    if scm_type == "random":
        if g is None:
            raise ValueError("Graph must be provided for random scm")
    else:
